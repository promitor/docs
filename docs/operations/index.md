# Operating Promitor

Here is an overview of how you can operate Promitor.

- [Health](health.md)
  - [Consuming the health endpoint](health.md#consuming-the-health-endpoint)
- [Discovery](discovery.md)
  - [Subscription](discovery.md#subscription)
  - [Resource Groups](discovery.md#resource-groups)
- [Performance](performance.md)
  - [Scraping Prometheus endpoint](performance.md#scraping-prometheus-endpoint)
  - [Scraping Azure Monitor](performance.md#scraping-azure-monitor)
- [System](system.md)
  - [Consuming the System endpoint](system.md#consuming-the-system-endpoint)
  - [Exploring our REST APIs](system.md#exploring-our-rest-apis)
- [Integrations](integrations.md)
  - [Azure Resource Manager API - Consumption & Throttling](integrations.md#azure-resource-manager-api---consumption--throttling)
  - [Azure Resource Graph](integrations.md#azure-resource-graph)
  - [Azure Monitor](integrations.md#azure-monitor)
